...
#include "HDMI_Lib/HDMI_RX/HDMI_RX.h"
#include "HDMI_Lib/HDMI_RX/it6605.h"
...
void THDMI_TX_I2S_Start(void){
     IOWR(I2S_TX_BASE, I2S_REG_CSR, 0x01);
}

void THDMI_TX_I2S_Stop(void){
     IOWR(I2S_TX_BASE, I2S_REG_CSR, 0x00);
}
...
void THDMI_TX_I2S_Play(alt_u32 szData[], int nNum){
    ...
    for(i=0;i<nNum;i++){
        ...
        while(Status & I2S_CSR_FIFO_FULL){
            Status = IORD(I2S_TX_BASE, I2S_REG_CSR);
        } 
        Value = szData[i];
        IOWR(I2S_TX_BASE, I2S_REG_FIFO, Value);
    }        
}
...
void i2s_isr(void* context, alt_u32 id){
    ...
    if (gbPlayTone){
        int nPushNum,i;
        nPushNum = 256 - IORD(I2S_TX_BASE, I2S_REG_FIFO_USEDW);
        for(i=0;i<nPushNum;i++){
            IOWR(I2S_TX_BASE, I2S_REG_FIFO, szWave[nIndex]);
            nIndex++;
    ...
    // clear irq flag
    IOWR(I2S_TX_BASE, I2S_REG_IRQ_CLEAR, 0x00);
}
